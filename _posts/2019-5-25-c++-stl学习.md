...
- [容器](#容器)
    ...
    - [容器适配器](#容器适配器)
    - [关联容器](#关联容器)
        - [unoredred_xxx的桶管理](#unoredred_xxx的桶管理)
        - [类型别名](#类型别名)
        - [遍历](#遍历)
        - [算法](#算法)
        - [操作](#操作)
...
注意: queque stack没有迭代器，也就是没有begin(),end()这种遍历操作.
...
###　unoredred_xxx的桶管理
管理就是通常的hash数据结构，比如链表数组，不过这里的每个链表叫做桶了:
...
关联容器只能适用read only类的泛型算法,因为key_type是const的，不能修改内容.

常用的是find.

也可以copy这种复制值出来，inserter这种插值进去
...
new的内存分配和构造通常在一起,new完，对象也都构造完了.

allocator可以做到: 分配是分配，构造是构造.　有的情况也许只要分配，不想那么快构造,
尤其是内存大，又需要很多冗余的场景，new并不合适:
```cpp
